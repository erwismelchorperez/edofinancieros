...
class Clasificadores:
    ...
    def PrediccionModelos(self, xtest):
        ...
        self.ypredRF = self.scaler.inverse_transform(self.ypredRF.reshape(1,-1))[0]
        """
        ...
        print("MLP:  ",self.ypredRF)
        #print("lstm: ",self.ypredlstm)
        #print("GRU:   ",self.ypredgru)

        self.meansquerederror = [mean_squared_error(self.ypredRF,ytest)]
        self.r2score = [r2_score(self.ypredRF,ytest)]
        self.errorabsolutomedio = [median_absolute_error(self.ypredRF,ytest)]
        ...
        for i in range(len(self.dataset)):
            ...
            if self.dataset.iloc[i]['NIVELv2'] == 'Predictor':
                ...
                for j in range(len(self.Pruebas)):
                    ...
                    elementos[j*2+contador+1] = clasifier.ypredRF[j]
...
for name in edo.DatosGraficar.keys():
    #print(name)
    edo.DatosGraficar[name].to_excel("./DatosGraficarGARF/"+name+"_"+clasificador+".xlsx")
