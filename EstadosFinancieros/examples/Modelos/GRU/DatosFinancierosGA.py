...
class GeneticAlgorithmLSTM:
    ...
    def initialize_population(self):
        ...
        for _ in range(self.population_size):
            individual = {
                'num_units': np.random.randint(50, 12),
...
class GeneticAlgorithmGRU:
    ...
    def initialize_population(self):
        ...
        for _ in range(self.population_size):
            individual = {
                'num_units': np.random.randint(50, 120),
                'num_layers': np.random.randint(1, 4),
                'learning_rate': 10**np.random.uniform(-4, -2),
                'batch_size': np.random.randint(16, 128)
            }
    ...
    def fitness(self, individual, X_train, y_train, X_val, y_val):
        model = Sequential()
        for _ in range(individual['num_layers']):
            model.add(GRU(units=individual['num_units'], activation='relu', return_sequences=True, input_shape=(X_train.shape[0], X_train.shape[1])))
        model.add(GRU(units=individual['num_units'], activation='relu'))
        model.add(Dense(1))
...
class Clasificadores:
    def __init__(self, clasificador):
        ...
        self.ypredlstm = []
        self.ypredgru = []
        self.meansquerederror = np.zeros(6)
        self.r2score = np.zeros(6) # es coeficiente
        self.errorabsolutomedio = np.zeros(6)
    ...
    def EntranientoModelos(self, xtrain):
        ...
        if self.clasificador == "RL":
            ...
            if self.clasificador == "DT":
                ...
                self.decisiontreemodel = DecisionTreeRegressor(
                ...
                self.decisiontreemodel.train(xtrain, ytrain)
            ...
            if self.clasificador == "LSTM":
                ...
                self.modellstm.train(xtrain, ytrain, epochs=10, batch_size=best_individual['batch_size'])
            if self.clasificador == "GRU":
                    #GRU
                ga = GeneticAlgorithmGRU(population_size=10, mutation_rate=0.1, generations=5)
                population = ga.initialize_population()
                best_population = ga.evolve(population, x_train, y_train, x_test, y_test)
    ...
    def PrediccionModelos(self, xtest):
        ...
        self.meansquerederror = [mean_squared_error(self.ypredgru,ytest)]
        self.r2score = [r2_score(self.ypredgru,ytest)]
        self.errorabsolutomedio = [median_absolute_error(self.ypredgru,ytest)]
    ...
    def AnalizarPrediccion(self):
        ...
        for i in range(len(self.dataset)):
            ...
            if self.dataset.iloc[i]['NIVELv2'] == 'Predictor':
                ...
                self.DatosGraficar[cadena].to_excel("./DatosGraficarGAGRU/"+cadena+"_"+self.clasificador+".xlsx")
                ...
                for j in range(len(self.Pruebas)):
                    ...
                    elementos[j*2+contador+1] = clasifier.ypredgru[j]
...
for name in edo.DatosGraficar.keys():
    #print(name)
    edo.DatosGraficar[name].to_excel("./DatosGraficarGAGRU/"+name+"_"+clasificador+".xlsx")
